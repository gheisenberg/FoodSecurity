...
def normalize_resize(img: str, img_path: str, input_height: str, input_width: str, clipping_values: list,
    ...
    # Create new geotiff
    dst_ds = gdal.GetDriverByName('GTiff').Create(os.path.join(img_path, img), input_height, input_width, len(array),
                                                  gdal.GDT_Byte)
...
def main(img_path: str, input_height: str, input_width: str, clipping_values: list, channels: list):
    ...
    train_val_stds = calc_std(train_val_means, img_path, X_train_val, input_height, input_width, clipping_values,
                              channels)
    ...
    for image in tqdm(X_train_val):
        normalize_resize(image, img_path, input_height, input_width, clipping_values, train_val_means, train_val_stds)

    for image in tqdm(X_train_val):
        normalize_resize(image, img_path, input_height, input_width, clipping_values, test_means, test_stds)
