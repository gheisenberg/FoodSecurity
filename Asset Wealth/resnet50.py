...
def load_resnet50(img_w: int, img_h: int, channels: int):
    """
    Build a resnet50 model with the required input shape
    Args:
        img_w: Pixel width for input images
        img_h: Pixel height for input images
        channels: number of channels for input images
    ...
    Returns:
        model (keras.models.Model): Resnet50 Model
    """
    ...
    hs_inputs = Input(shape=(img_h, img_w, channels), name='input')
    activation = 'softmax'
    repetitions = [3, 4, 6, 3]
    initial_filters = 64
    initial_strides = (2, 2)
    ...
    x = _conv_bn_relu(filters=initial_filters, kernel_size=initial_kernel_size,
                          strides=initial_strides)(hs_inputs)
    ...
    x = layers.Dense(128, activation='relu')(x)
    x = layers.Dense(1)(x)
    model = Model(inputs=hs_inputs, outputs=x)
    return model