...
def final_ds_droping_cols(df, drop_meta=False, drop_6_7_y_FS=True, drop_food_help=True, drop_IPC_wo_food_help=False, drop_perc=45, drop_region=True, 
                ...
                use_NAN_amount_and_replace_NANs_in_categorical=True, retain_year=True, retain_month=True, 
        ...
        for col_base, l in dic.items():
            if len(l) == 2 and drop_one_of_double_categorical_cols:
                ...
                if len(df[col1].dropna()) > len(df[col2].dropna()):
                    ...
                    elif 'appicable' in l and 'not applicable' in l:
        ...
        if verbose > 1:
            ...
            for c in drop_perc_cols:
                ...
                if ': ' in c_base:
                    c_base = c.rsplit(': ', 1)[0]
                if c_base not in seen:
                    print(f'Dropped perc: {c_base}: {round(len(df_in[c].dropna()) / len(df_in[c]) * 100, 1)}%')
                    seen.add(c_base)
            ...
            for c in df.columns:
                ...
                if c_base not in seen:
                    if verbose == 2:
                        print(f'Remaining: {c}: {round(len(df_in[c].dropna()) / len(df_in[c]) * 100, 1)}% (and subsets)')
                        seen.add(c_base)
                    else:
